...
object MyModule {
  ...
  /**
    * Exercise 2.1
    *
    * Write a recursive function to get the n th Fibonacci number. The first two Fibonacci numbers are 0 and 1.
    * The n th number is always the sum of the previous two—the sequence begins 0, 1, 1, 2, 3, 5.
  ...
  def fib(n: Int): Int = {
    ...
    def go(n: Int, prev: Int, cur: Int): Int = {
      if (n == 0) prev else go(n - 1, cur, cur + prev)
    }
...
object PolymorphicFunctions {
  ...
  /**
    * Exercise 2.2
    * Implement isSorted , which checks whether an Array[A] is sorted according to a given comparison function gt.
    */
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    ...
    if (as.length == 0) true else go(1, as(0))
  }

  // Polymorphic functions are often so constrained by their type that they only have one implementation! Here's an example:
  def partial1[A, B, C](a: A, f: (A, B) => C): B => C =
    (b: B) => f(a, b)

  /**
    * Exercise 2.3
    * Implement curry, which converts a function f of two arguments (a,b) => c
    * into a function of one argument (b) => c that partially applies a to f.
    */
  ...
  /**
    ...
    * Uncurry converts a function f(a) that returns another function g(b) = c to a function h(a,b) = c
    */
